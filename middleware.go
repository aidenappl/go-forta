...
//   - Otherwise: tokens are validated remotely by calling /oauth/userinfo on the
...
// middleware additionally calls /oauth/userinfo so that the full User profile
...
func (c *Client) Protected(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if c.cfg.JWTSigningKey != "" {
			...
			// ── Remote validation via /oauth/userinfo ────────────────────────
